...
Заходи в чат, мы ждем именно тебя🫵🏼"
    ...
    def how_work_about(self):
        return '❗️Встречи Master`s utro проходят три раза в неделю:
...
Также Masters включает в себя крутую реферальную систему👇🏼'
    ...
    def referal_system_about(self):
        return '🔥Реферальная система Masters - это настоящее сокровище, кроме того, что благодаря реферальной ссылке вам удобно поделиться чатом с близкими и друзьями, вы еще и получите до 71%❗️от дохода за каждого приведенного участника💸
...
- Утренние лекции для положительного заряда на целый день;
- Групповые обсуждения наиболее интересных вопросов вместе с 
- В чате ты найдешь много единомышленников, с которыми будешь постоянно расти;
- Поддержка ментора 24/7;
- Удобная реферальная система для приглашения друзей.
...
За кнопкой присоединиться под этим текстом👇🏼'
